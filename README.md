...
- **会话与登录保护**：数据库只保存访问令牌摘要，改密撤销全部会话，并通过 Redis 限制同一账号与客户端来源的连续登录失败。
- **完整应用能力**：包含登录与角色边界、新闻管理、Redis 缓存、收藏、浏览历史、AI 摘要和文章问答。
- **工程化交付**：Python 3.12、非 root 容器、MySQL/Redis 健康检查、GitHub Actions 和 50 项自动化测试。
...
- 后续优先完善高成本 AI 接口限流、MySQL 集成测试与备份恢复演练，不将尚未实现的 Agent 能力写入项目声明。
...
- [登录限流与会话安全](docs/auth-session-security.md)
- [登录与会话模块就绪审查](docs/auth-security-readiness-audit.md)
